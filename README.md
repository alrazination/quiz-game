...
3. Create three tabs at the bottom, named **exactly**: `Participants`,
   `Questions`, `Results`.
4. In **Participants**, row 1 (headers): `player_code | name | team | active`
   Then add one row per participant, e.g.:
   ```
   A1001 | Sarah Smith | Sales     | TRUE
   A1002 | John Brown  | Marketing | TRUE
   ```
5. In **Questions**, row 1 (headers):
   ...
   `C`, or `D`.
6. Leave **Results** with just the header row for now — the game fills it in
   automatically after the event: `rank | player_code | name | team | score`
...
2. Click **Load from Sheet** — this pulls your Participants and Questions
   from the Google Sheet through the Apps Script bridge and caches them in
   the Durable Object.
...
4. On a few phones, open the player URL, scan the QR code shown on the host
   screen, or type it in manually, and enter a real `player_code` from your
   sheet.
5. Click **Start game**, then **Next question**, and confirm: the question
   ...
   disables the buttons and shows "ANSWER RECEIVED", the countdown ends, the
   correct answer is revealed, and **Show leaderboard** shows scores.
6. Try turning a phone's Wi-Fi off and back on mid-game — it should
   reconnect and keep its score.
...
it runs on GitHub's servers, not yours.

1. In your Google Sheet, open **Extensions → Apps Script**, pick
   `addSimulationParticipants` from the function dropdown in the toolbar,
   and click **Run**. This adds 2,000 test rows (`SIM0001`...`SIM2000`) to
   your Participants tab so the simulator has valid codes to use.
2. In your repo's **Actions** tab, click **Load test** → **Run workflow**.
3. Fill in:
...
4. Click **Run workflow**. Meanwhile, open the host screen, **Load from
   Sheet**, **Start game**, and step through questions — watch the connected
   player count and leaderboard update as the simulated players "answer."
5. Afterward: click **Restart game** on the host screen to clear simulated
   scores, then in Apps Script run `removeSimulationParticipants` to delete
   the SIM rows before the real event.
...
3. Display the host screen (join screen + QR code) while people arrive and
   scan in on their phones.
4. Watch the connected-player count climb.
5. **Start game**, then **Next question** for each question, in order.
6. After each reveal, **Show leaderboard** if you want to pause on standings,
   then **Next question** again.
7. After the last question, **Show final results** — this reveals the
